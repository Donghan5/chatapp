import { Controller, Post, Body, Request, UseGuards, Get, Param, ParseIntPipe, Delete, Patch } from "@nestjs/common";
...
export class ChatRoomsController {
    ...
    @Patch(':id')
    updateChatRoom(@Request() req, @Param('id', ParseIntPipe) id: number, @Body() updateChatRoomDto: UpdateChatRoomDto) {
        return this.chatRoomService.updateChatRoom(id, req.user.id, updateChatRoomDto);
    }
    ...
    @Post(':id/invite')
    inviteUserToChatRoom(@Request() req, @Param('id', ParseIntPipe) id: number, @Body() inviteUserDto: InviteUserDto) {
        return this.chatRoomService.inviteUserToChatRoom(inviteUserDto.targetUserId, id);
    }
}